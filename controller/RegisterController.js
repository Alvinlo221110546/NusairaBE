...
export const registerUser = async (req, res) => {
  const { name, username, email, password, no_hp, pekerjaan } = req.body;

  if (!name || !username || !email || !password || !no_hp || !pekerjaan) {
  ...
  try {
    ...
    const userId = await Register.create(name, username, email, hashedPassword, "user", no_hp, pekerjaan);
